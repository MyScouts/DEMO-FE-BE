...
import { changeCoinToPointService, IPaymentInfo, updateCoinService, withdrawCoinService } from '../../reponsitory/UserService';
...
const Withdrawal = (props: IProps) => {
    const schema = yup.object({
        quantity: yup.number().required().positive().integer().max(props.totalPoint),
    }).required();
    ...
    const onSubmit = handleSubmit(async (data: any) => {
        if (props.totalPoint - usedPoint >= 0) {
            const result = await withdrawCoinService({ point: data.quantity, methodId: data.method });
            if (result) {
                props.handleClose();
                router.reload();
                alterSuccess('Rút tiền thành công')
                return;
            }

            alterFail('Rút tiền thất bại')
            return;
        } else {
    ...
    return (
        <Modal
            ...
            <Box sx={container}>
                <h1 style={{ fontSize: "1.5rem", fontWeight: "bold", textTransform: "uppercase" }}>Withdrawal</h1>
